<template>
  ...
  <thead>
    ...
    <tr v-for="(item, i) in userlist" :key="i">
        ...
        <th>{{item.major}}</th>
        <th>{{item.status}}</th>
        <th><button class="btn btn-primary" data-dismiss="modal" @click="editProfileUser(item.rsuid)">เเก้ไข</button></th>
      </tr>
        <!-- <v-data-table
    :headers="headers"
    :items="desserts"
...
export default {
  ...
  methods: {
    ...
    editProfileUser (data) {
      ...
      let _data = []
      console.log(rsuId)
      db.collection('MEMBER_TABLE').where('rsuid', '==', rsuId).get().then(userShow => {
        userShow.forEach(doc => {
